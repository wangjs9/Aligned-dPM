...
class Model(BaseModel, BertForSequenceClassification):
    ...
    def get_prompt(self, name, bsz=None):
        old_bsz = bsz
        ...
        for i, key_val_enc in enumerate(past_key_values_enc):
            ...
            temp["encoder_prompt"] = {
                ...
                "prev_key_padding_mask": torch.zeros(bsz_enc, seqlen).to(self.device).bool(),
            }
            result.append(temp)
    ...
    def forward(
        ...
        distribution = {}
        past_prompt = self.get_prompt("prediction", bsz)
        outputs = self.bert(
            input_ids,
            past_prompt,
    ...
    def comp_loss(self, distribution):
        prediction, major_label = distribution["prediction"], distribution["label"]
