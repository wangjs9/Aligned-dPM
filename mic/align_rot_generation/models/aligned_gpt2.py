...
class AlignedGPT2Model(GPT2Model):
    def forward(
            self,
            input_ids=None,
        ...
        if input_ids != None and inputs_embeds != None:
        ...
        if self.config.add_cross_attention and encoder_hidden_states != None:
...
class AlignedGPT2LMHeadModel(GPT2LMHeadModel):
    def __init__(self, config: GPT2Config):
        super().__init__(config)
        self.transformer = AlignedGPT2Model(config)
    ...
    def forward(
        ...
        """
        return_dict = return_dict if return_dict != None else self.config.use_return_dict
        batch_size = input_ids.size(0) if input_ids != None else encoder_hidden_states.size(0)
        ...
        if lm_logits.size(0) > batch_size:
            ...
            all_lm_logits = shift_logits.view(batch_size, -1, lm_logits.size(1), lm_logits.size(2))
            ...
            candidate_labels = candidate_labels[..., 1:].contiguous()
            if score_mode == "log":
